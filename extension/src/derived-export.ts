...
export function renderDerivedExport(record: CaptureRecord<Record<string, unknown>>, format: DerivedFormat): DerivedExportResult {
    ...
    return { fileName: `${safeTitle}--${id}.md`, mimeType: 'text/markdown;charset=utf-8', content: `# ${title}\n\n${blocks.map(block => `#### ${block.author}:\n\n${block.content}`).join('\n\n')}` }
}
...
function messageText(message: ConversationMessage): string {
    ...
    if (content.content_type === 'multimodal_text') return content.parts?.map(part => typeof part === 'string' ? part : '[image]').join('\n') || ''
